...
export async function getCustomer360(customerId: string) {
  ...
  const [customer, contacts, invoices, quotes, orders, deliveryNotes, payments, reminders] = await Promise.all([
    ...
    tid ? supabase.from('sales_orders').select('*').eq('customer_id', customerId).eq('tenant_id', tid).order('order_date', { ascending: false }) : supabase.from('sales_orders').select('*').eq('customer_id', customerId).order('order_date', { ascending: false }),
    tid ? supabase.from('delivery_notes').select('*').eq('customer_id', customerId).eq('tenant_id', tid).order('delivery_date', { ascending: false }) : supabase.from('delivery_notes').select('*').eq('customer_id', customerId).order('delivery_date', { ascending: false }),
