...
// ============================================
// EXCEL EXPORT UTILITY (.xls HTML table)
// ============================================
export function exportToExcel(filename: string, headers: string[], rows: (string | number)[][]) {
  const tableHeader = headers.map((h) => `<th style="background:#4472C4;color:white;font-weight:bold;padding:4px;border:1px solid #ccc;">${String(h).replace(/</g, '&lt;')}</th>`).join('')
  const tableRows = rows.map((r) =>
    `<tr>${r.map((cell) => {
      const s = String(cell ?? '').replace(/</g, '&lt;').replace(/>/g, '&gt;')
      return `<td style="padding:4px;border:1px solid #ddd;mso-number-format:'@';">${s}</td>`
    }).join('')}</tr>`
  ).join('')
  const html = `<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns="http://www.w3.org/TR/REC-html40"><head><meta charset="UTF-8"><!--[if gte mso 9]><xml><x:ExcelWorkbook><x:ExcelWorksheets><x:ExcelWorksheet><x:Name>Sheet1</x:Name><x:WorksheetOptions><x:DisplayGridlines/></x:WorksheetOptions></x:ExcelWorksheet></x:ExcelWorksheets></x:ExcelWorkbook></xml><![endif]--></head><body><table><thead><tr>${tableHeader}</tr></thead><tbody>${tableRows}</tbody></table></body></html>`
