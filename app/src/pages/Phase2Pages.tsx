...
export function ProspectsPage() {
  ...
  async function handleDelete(id: string) { if (!window.confirm(tCommon('common.confirmDelete'))) return; try { await deleteProspect(id); await loadData() } catch (e: any) { toast('error', tCommon('common.error'), e.message) } }
  ...
  return (
    <div>
      ...
      {showForm && (
        <Card className="p-4 mb-4 space-y-3">
          ...
          <div className="flex gap-2">
            <Button onClick={handleCreate}>{tCommon('common.save')}</Button>
      ...
      {loading ? <SkeletonTable /> : items.length === 0 ? <EmptyState title={t('prospects.empty')} /> : (
        <Table headers={[t('prospects.name'), t('prospects.email'), t('prospects.phone'), t('prospects.status'), t('common.actions')]}>
          {items.map(p => (
            <TableRow key={p.id}>
              <TableCell>{p.name}</TableCell>
              <TableCell>{p.email || '-'}</TableCell>
              <TableCell>{p.phone || '-'}</TableCell>
              <TableCell><Badge>{t(`prospects.status.${p.status}`)}</Badge></TableCell>
              <TableCell>
...
export function RepresentativesPage() {
  ...
  async function handleDelete(id: string) { if (!window.confirm(tCommon('common.confirmDelete'))) return; try { await deleteSalesRepresentative(id); await loadData() } catch (e: any) { toast('error', tCommon('common.error'), e.message) } }
    ...
    <div>
      ...
      {showForm && (
        <Card className="p-4 mb-4 space-y-3">
          ...
          <div className="flex gap-2"><Button onClick={handleCreate}>{tCommon('common.save')}</Button><Button variant="secondary" onClick={() => setShowForm(false)}>{tCommon('common.cancel')}</Button></div>
        </Card>
      )}
      {loading ? <SkeletonTable /> : items.length === 0 ? <EmptyState title={t('reps.empty')} /> : (
        <Table headers={[t('reps.name'), t('reps.email'), t('reps.phone'), t('reps.commission'), t('reps.territory'), t('common.actions')]}>
...
export function WarehouseLocationsPage() {
  ...
  async function handleDelete(id: string) { if (!window.confirm(tCommon('common.confirmDelete'))) return; try { await deleteWarehouseLocation(id); await loadData() } catch (e: any) { toast('error', tCommon('common.error'), e.message) } }
...
export function PickListPage() {
  ...
  return (
    <div>
      ...
      {showForm && (<Card className="p-4 mb-4 space-y-3"><Input placeholder={t('picking.number')} value={form.number} onChange={e => setForm({ ...form, number: e.target.value })} /><div className="flex gap-2"><Button onClick={handleCreate}>{tCommon('common.save')}</Button><Button variant="secondary" onClick={() => setShowForm(false)}>{tCommon('common.cancel')}</Button></div></Card>)}
      {loading ? <SkeletonTable /> : items.length === 0 ? <EmptyState title={t('picking.empty')} /> : (
        <Table headers={[t('picking.number'), t('picking.status'), t('picking.date'), t('common.actions')]}>
...
export function SerialNumbersPage() {
  ...
  async function handleDelete(id: string) { if (!window.confirm(tCommon('common.confirmDelete'))) return; try { await deleteProductSerialNumber(id); await loadData() } catch (e: any) { toast('error', tCommon('common.error'), e.message) } }
...
export function ProductBatchesPage() {
  ...
  async function handleDelete(id: string) { if (!window.confirm(tCommon('common.confirmDelete'))) return; try { await deleteProductBatch(id); await loadData() } catch (e: any) { toast('error', tCommon('common.error'), e.message) } }
...
export function DocumentTemplatesPage() {
  ...
  async function handleDelete(id: string) { if (!window.confirm(tCommon('common.confirmDelete'))) return; try { await deleteDocumentTemplate(id); await loadData() } catch (e: any) { toast('error', tCommon('common.error'), e.message) } }
...
export function DeliverySchedulePage() {
  ...
  async function handleDelete(id: string) { if (!window.confirm(tCommon('common.confirmDelete'))) return; try { await deleteDeliverySchedule(id); await loadData() } catch (e: any) { toast('error', tCommon('common.error'), e.message) } }
...
export function ProductSubstitutesPage() {
  ...
  async function handleDelete(id: string) { if (!window.confirm(tCommon('common.confirmDelete'))) return; try { await deleteProductSubstitute(id); await loadData() } catch (e: any) { toast('error', tCommon('common.error'), e.message) } }
