  ...
  return (
    <div>
      ...
      {loading ? <SkeletonTable /> : items.length === 0 ? <EmptyState title={t('prospects.empty')} /> : (
        <Table headers={[t('prospects.name'), t('prospects.email'), t('prospects.phone'), t('prospects.status'), tCommon('table.actions')]}>
          {items.map(p => (
            <TableRow key={p.id}>
              ...
              <TableCell>
                <div className="flex gap-1">
                  ...
                  <Button size="sm" variant="danger" onClick={() => handleDelete(p.id)} ariaLabel={tCommon('actions.delete')}><Trash2 className="w-4 h-4" aria-hidden="true" /></Button>
                </div>
              </TableCell>
            </TableRow>
          ))}
  ...
  return (
    <div>
      ...
      {loading ? <SkeletonTable /> : items.length === 0 ? <EmptyState title={t('reps.empty')} /> : (
        <Table headers={[t('reps.name'), t('reps.email'), t('reps.phone'), t('reps.commission'), t('reps.territory'), tCommon('table.actions')]}>
          {items.map(r => (<TableRow key={r.id}><TableCell>{r.name}</TableCell><TableCell>{r.email || '-'}</TableCell><TableCell>{r.phone || '-'}</TableCell><TableCell>{r.commission_rate}%</TableCell><TableCell>{r.territory || '-'}</TableCell><TableCell><Button size="sm" variant="danger" onClick={() => handleDelete(r.id)} ariaLabel={tCommon('actions.delete')}><Trash2 className="w-4 h-4" aria-hidden="true" /></Button></TableCell></TableRow>))}
        </Table>
      )}
    </div>
  )
  ...
  return (
    <div>
      ...
      {loading ? <SkeletonTable /> : items.length === 0 ? <EmptyState title={t('picking.empty')} /> : (
        <Table headers={[t('picking.number'), t('picking.status'), t('picking.date'), tCommon('table.actions')]}>
  ...
  return (
    <div>
      ...
      {loading ? <SkeletonTable /> : items.length === 0 ? <EmptyState title={t('serials.empty')} /> : (
        <Table headers={[t('serials.product'), t('serials.number'), t('serials.status'), tCommon('table.actions')]}>
          {items.map(s => (<TableRow key={s.id}><TableCell>{s.products?.name || '-'}</TableCell><TableCell>{s.serial_number}</TableCell><TableCell><Badge>{t(`serials.statuses.${s.status}`)}</Badge></TableCell><TableCell><Button size="sm" variant="danger" onClick={() => handleDelete(s.id)} ariaLabel={tCommon('actions.delete')}><Trash2 className="w-4 h-4" aria-hidden="true" /></Button></TableCell></TableRow>))}
        </Table>
      )}
    </div>
  )
  ...
  return (
    <div>
      ...
      {loading ? <SkeletonTable /> : items.length === 0 ? <EmptyState title={t('schedules.empty')} /> : (
        <Table headers={[t('schedules.customer'), t('schedules.product'), t('schedules.frequency'), t('schedules.quantity'), t('schedules.startDate'), tCommon('table.actions')]}>
    ...
    <div>
      ...
      {loading ? <SkeletonTable /> : items.length === 0 ? <EmptyState title={t('substitutes.empty')} /> : (
        <Table headers={[t('substitutes.product'), t('substitutes.substitute'), t('substitutes.priority'), tCommon('table.actions')]}>
          {items.map(s => (<TableRow key={s.id}><TableCell>{s.produit?.name || '-'}</TableCell><TableCell>{s.substitut?.name || '-'}</TableCell><TableCell>{s.priority}</TableCell><TableCell><Button size="sm" variant="danger" onClick={() => handleDelete(s.id)} ariaLabel={tCommon('actions.delete')}><Trash2 className="w-4 h-4" aria-hidden="true" /></Button></TableCell></TableRow>))}
        </Table>
      )}
    </div>
  )
