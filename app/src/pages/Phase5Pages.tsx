...
export function AssetFamiliesPage() {
  ...
  async function handleDelete(id: string) { if (!window.confirm(tCommon('common.confirmDelete'))) return; try { await deleteAssetFamily(id); await loadData() } catch (e: any) { toast('error', tCommon('common.error'), e.message) } }
...
export function BatchDisposalPage() {
  ...
  return (
    <div>
      ...
      {showForm && (<Card className="p-4 mb-4 space-y-3"><div className="grid grid-cols-2 gap-3"><Input placeholder={t('batchDisposals.number')} value={form.batch_number} onChange={e => setForm({ ...form, batch_number: e.target.value })} /><Input type="date" value={form.disposal_date} onChange={e => setForm({ ...form, disposal_date: e.target.value })} /></div><div className="flex gap-2"><Button onClick={handleCreate}>{tCommon('common.save')}</Button><Button variant="secondary" onClick={() => setShowForm(false)}>{tCommon('common.cancel')}</Button></div></Card>)}
      {loading ? <SkeletonTable /> : items.length === 0 ? <EmptyState title={t('batchDisposals.empty')} /> : (
        <Table headers={[t('batchDisposals.number'), t('batchDisposals.date'), t('batchDisposals.assets'), t('batchDisposals.proceeds'), t('batchDisposals.gainLoss'), t('batchDisposals.status'), t('common.actions')]}>
...
export function AssetFromEntryPage() {
  ...
  async function handleCreate() {
    ...
    try {
      ...
      const { data: entry, error: eErr } = await supabase.from('journal_entries').select('*, journal_lines(*)').eq('id', entryId).single()
      if (eErr) throw eErr
