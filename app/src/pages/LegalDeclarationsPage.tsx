...
const [declarations, setDeclarations] = useState<LegalDeclaration[]>([])
  ...
  return (
    <div>
      <Breadcrumb items={[{ label: 'Paie & RH' }, { label: 'Déclarations' }]} />
      <PageHeader title="Déclarations sociales & fiscales" subtitle={`${declarations.length} déclaration(s)`}
        action={<Button onClick={() => setShowForm(true)}><Plus className="w-4 h-4" /> Nouvelle déclaration</Button>} />

      <div className="grid grid-cols-3 gap-4 mb-6">
        <Card><div className="p-4"><p className="text-sm text-[var(--color-text-secondary)]">En attente</p><p className="text-2xl font-bold text-[var(--color-warning)]">{pendingCount}</p></div></Card>
        <Card><div className="p-4"><p className="text-sm text-[var(--color-text-secondary)]">En retard</p><p className="text-2xl font-bold text-[var(--color-danger)]">{lateCount}</p></div></Card>
        <Card><div className="p-4"><p className="text-sm text-[var(--color-text-secondary)]">Transmises</p><p className="text-2xl font-bold text-[var(--color-success)]">{declarations.filter((d) => d.status === 'submitted').length}</p></div></Card>
      </div>

      <div className="flex gap-3 mb-4 items-end">
        <div className="w-48">
          <Select label="Statut" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} options={[
            { value: '', label: 'Tous' }, { value: 'pending', label: 'En attente' }, { value: 'submitted', label: 'Transmise' },
            { value: 'late', label: 'En retard' }, { value: 'cancelled', label: 'Annulée' },
          ]} />
        </div>
      </div>

      {loading ? <SkeletonTable rows={4} cols={6} /> : declarations.length === 0 ? (
        <EmptyState icon={<ShieldCheck className="w-8 h-8" />} title="Aucune déclaration" description="Créez votre première déclaration."
          action={<Button onClick={() => setShowForm(true)}><Plus className="w-4 h-4" /> Nouvelle déclaration</Button>} />
      ) : (
        <Card>
          <Table headers={['N°', 'Type', 'Période', 'Échéance', 'Montant', 'Statut', 'Actions']}>
            {declarations.map((d) => (
              <TableRow key={d.id}>
                <TableCell className="font-mono text-xs">{d.number}</TableCell>
                <TableCell className="text-xs">{typeLabels[d.declaration_type] || d.declaration_type}</TableCell>
                ...
                <TableCell><Badge variant={statusBadge[d.status] || 'neutral'}>{statusLabels[d.status] || d.status}</Badge></TableCell>
                <TableCell>
                  <div className="flex gap-1">
                    {d.status === 'pending' && (
                      <button onClick={() => handleSubmit(d.id)} className="p-1.5 rounded hover:bg-[var(--color-neutral-100)] text-[var(--color-success)]" title="Marquer transmise"><Send className="w-4 h-4" /></button>
                    )}
    ...
    <div className="fixed inset-0 bg-black/50 z-[9990] flex items-center justify-center p-4">
      <div className="card shadow-2xl overflow-hidden" style={{ width: '100%', maxWidth: '32rem' }}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--color-border)]">
          <h2 className="text-lg font-semibold">Nouvelle déclaration</h2>
          <button onClick={onClose} className="p-1 rounded hover:bg-[var(--color-neutral-100)]"><X className="w-5 h-5" /></button>
        </div>
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <Select label="Type" value={declarationType} onChange={(e) => setDeclarationType(e.target.value)} options={[
            { value: 'dsn', label: 'DSN' }, { value: 'urssaf', label: 'URSSAF' }, { value: 'dgt', label: 'DGT' },
            { value: 'ifrs', label: 'IFRS' }, { value: 'other', label: 'Autre' },
          ]} />
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-[var(--color-text-secondary)] mb-1">Mois</label>
            ...
            <Input label="Année" type="number" required value={periodYear} onChange={(e) => setPeriodYear(Number(e.target.value))} />
          </div>
          <Input label="Date d'échéance" type="date" required value={dueDate} onChange={(e) => setDueDate(e.target.value)} />
          <Input label="Montant" type="number" step="0.01" value={amount} onChange={(e) => setAmount(Number(e.target.value))} />
