...
export function BudgetTrackingPage() {
  ...
  function exportCSV() {
    const headers = [t('budgetTracking.account'), t('budgetTracking.label'), t('budgetTracking.budgeted'), t('budgetTracking.actual'), t('budgetTracking.committed'), t('budgetTracking.available'), t('budgetTracking.variance'), t('budgetTracking.variancePercentage')]
  ...
  return (
    <div>
      <Breadcrumb items={[{ label: t('title'), path: '/reporting/budget' }, { label: t('budgetTracking.title') }]} />
      <PageHeader title={t('budgetTracking.title')} subtitle={t('budgetTracking.subtitleFull')} action={
        <div className="flex items-center gap-2">
          <Select value={yearFilter} onChange={(e) => handleFilterChange(e.target.value)} options={[
            { value: '', label: t('budgetTracking.allFiscalYears') },
            ...years.map(y => ({ value: y.id, label: y.code })),
          ]} />
          <Button variant="secondary" onClick={exportCSV}><Download className="w-4 h-4 mr-2" /> {t('budgetTracking.exportCsv')}</Button>
        </div>
      } />

      {items.length === 0 ? (
        <EmptyState title={t('budgetTracking.noBudget')} description={t('budgetTracking.noBudgetDesc')} />
      ) : (
        <Card>
          <Table headers={[t('budgetTracking.account'), t('budgetTracking.label'), t('budgetTracking.budgeted'), t('budgetTracking.actual'), t('budgetTracking.committed'), t('budgetTracking.available'), t('budgetTracking.variance'), t('budgetTracking.variancePercentage'), t('budgetTracking.status')]}>
            {items.map((item) => (
              <TableRow key={item.id}>
                ...
                <TableCell>
                  {(item.available || 0) >= 0 ? (
                    <span className="inline-flex items-center gap-1 text-green-600 text-sm"><TrendingUp className="w-3 h-3" /> {t('budgetTracking.inBudget')}</span>
                  ) : (
                    <span className="inline-flex items-center gap-1 text-red-600 text-sm"><TrendingDown className="w-3 h-3" /> {t('budgetTracking.overrun')}</span>
                  )}
                </TableCell>
              </TableRow>
            ))}
