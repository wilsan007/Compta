  ...
  return (
    <div>
      ...
      {loading ? <SkeletonTable /> : items.length === 0 ? <EmptyState title={t('creditLines.empty')} /> : (
        <Table headers={[t('creditLines.name'), t('creditLines.type'), t('creditLines.limit'), t('creditLines.used'), t('creditLines.rate'), t('creditLines.status'), tCommon('table.actions')]}>
  ...
  return (
    <div>
      ...
      {loading ? <SkeletonTable /> : items.length === 0 ? <EmptyState title={t('investments.empty')} /> : (
        <Table headers={[t('investments.name'), t('investments.type'), t('investments.initial'), t('investments.current'), t('investments.rate'), t('investments.status'), tCommon('table.actions')]}>
          {items.map(i => (<TableRow key={i.id}><TableCell>{i.name}</TableCell><TableCell>{t(`investments.types.${i.type}`)}</TableCell><TableCell>{formatCurrency(i.initial_amount)}</TableCell><TableCell>{formatCurrency(i.current_value)}</TableCell><TableCell>{i.interest_rate}%</TableCell><TableCell><Badge>{t(`investments.statuses.${i.status}`)}</Badge></TableCell><TableCell><Button size="sm" variant="danger" onClick={() => handleDelete(i.id)} ariaLabel={tCommon('actions.delete')}><Trash2 className="w-4 h-4" aria-hidden="true" /></Button></TableCell></TableRow>))}
        </Table>
      )}
    </div>
  )
  ...
  return (
    <div>
      ...
      {loading ? <SkeletonTable /> : items.length === 0 ? <EmptyState title={t('recurring.empty')} /> : (
        <Table headers={[t('recurring.description'), t('recurring.amount'), t('recurring.type'), t('recurring.frequency'), t('recurring.nextDate'), tCommon('table.actions')]}>
