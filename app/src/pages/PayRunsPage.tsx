...
const [payRuns, setPayRuns] = useState<PayRun[]>([])
  ...
  try { await updatePayRun(id, { status: status as any }); await loadData() } catch (err: any) { toast('error', 'Erreur', err.message || 'échec') }
  }

  async function handleDelete(id: string) {
    if (!window.confirm('Supprimer ce bulletin de paie ?')) return
    try { await deletePayRun(id); await loadData() } catch (err: any) { toast('error', 'Erreur', err.message || 'échec') }
  }

  async function handleGenerateJournal(id: string) {
    try {
      await generatePayrollJournal(id)
      toast('success', 'Succès', 'Écriture de journal générée avec succès')
      await loadData()
    } catch (err: any) {
  ...
  return (
    <div>
      ...
      <PageHeader
        ...
        subtitle="Générez et gérez vos bulletins de paie"
        action={<Button onClick={() => setShowForm(true)}><Plus className="w-4 h-4" /> Nouveau bulletin</Button>}
      />

      {loading ? (
        <SkeletonTable rows={4} cols={7} />
      ) : payRuns.length === 0 ? (
        <EmptyState icon={<Calendar className="w-8 h-8" />} title="Aucun bulletin" description="Générez votre premier bulletin de paie." action={<Button onClick={() => setShowForm(true)}><Plus className="w-4 h-4" /> Nouveau bulletin</Button>} />
      ) : (
        <Card>
          <Table headers={['Numéro', 'Période', 'Date paie', 'Employés', 'Brut', 'Net', 'Statut', 'Actions']}>
            {payRuns.map((pr) => (
              <TableRow key={pr.id}>
                ...
                <TableCell>
                  <div className="flex items-center gap-1">
                    <button onClick={() => handleGenerateJournal(pr.id)} className="p-1.5 rounded hover:bg-[var(--color-neutral-100)] text-[var(--color-primary)]" title="Générer écriture de journal"><FileText className="w-4 h-4" /></button>
      ...
      <div className="card shadow-2xl" style={{ width: '100%', maxWidth: '36rem' }}>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            <Input label="Début période" type="date" required value={periodStart} onChange={(e) => setPeriodStart(e.target.value)} />
            <Input label="Fin période" type="date" required value={periodEnd} onChange={(e) => setPeriodEnd(e.target.value)} />
            <Input label="Date de paie" type="date" required value={payDate} onChange={(e) => setPayDate(e.target.value)} />
          </div>
          <div className="p-3 rounded-lg bg-[var(--color-neutral-50)] space-y-1 text-sm">
            <div className="flex justify-between"><span className="text-[var(--color-text-secondary)]">Employés actifs:</span><span className="font-bold">{employees.length}</span></div>
            <div className="flex justify-between"><span className="text-[var(--color-text-secondary)]">Masse salariale brute:</span><span className="font-mono font-bold">{formatCurrency(grossTotal)}</span></div>
            <div className="flex justify-between"><span className="text-[var(--color-text-secondary)]">Charges sociales (~23%):</span><span className="font-mono text-[var(--color-danger)]">-{formatCurrency(taxTotal)}</span></div>
            <div className="flex justify-between border-t border-[var(--color-border)] pt-1"><span className="font-semibold">Net à payer:</span><span className="font-mono font-bold text-[var(--color-success)]">{formatCurrency(netTotal)}</span></div>
          </div>
          <div className="flex justify-end gap-3 pt-4 border-t border-[var(--color-border)]">
            <Button type="button" variant="secondary" onClick={onClose}>Annuler</Button>
            <Button type="submit" disabled={saving || employees.length === 0}>{saving ? '...' : 'Générer'}</Button>
          </div>
        </form>
      </div>
    </div>
