...
| 23/09 22h30 | **R-10 fait — la phase 1 est close, et deux lecteurs de relevés étaient faux sur de vrais fichiers.** Le plan demandait « relevés réels anonymisés par format (AUD-G03) » : écrits cette fois **d après les normes**, ils ont mis en évidence ce que des chaînes fabriquées ne pouvaient pas montrer. **CFONB 120** : le lecteur lisait une date ISO de 8 chiffres là où la norme écrit 6 (`JJMMAA`), un « sens » dans une colonne 105 qui est en réalité la **référence**, un libellé pris sur la zone du numéro d écriture et des drapeaux, et il ignorait l **échelle** (position 20) comme le fait que le montant est signé par son **14e caractère** (13 chiffres + un spécificateur qui porte signe et unité : `00000000019040}` = −190,40). Sur un fichier réel il perdait dates, montants et libellés, et datait du **jour** tout mouvement sans date — réécrit sur la brochure CFONB (01 ancien solde, 04 mouvement, 05 complément, 07 nouveau solde). **MT940** : la date de valeur est `MMDD` ; le lecteur n en sautait que six chiffres, donc `:61:2501010102D120,00NCHG` valait **102 et non 120**, sans un mot. La date de valeur n est désormais consommée que si le caractère suivant est bien un sens (C/D), `RC`/`RD` sont reconnus, et un solde sans devise (``:60F:C2501011000,00``) ne fait plus croire à une devise « 100 ». **OFX, qui n existait pas** : les deux générations (1.x SGML aux balises non fermées, 2.x XML), `CURDEF`, `LEDGERBAL`/`DTASOF`, `FITID`, le signe de `TRNAMT` qui fait foi et `TRNTYPE` seulement en repli. **Devise** : le relevé dont la devise diffère de celle du compte est **refusé** — mais seulement si la devise a été **réellement lue** (nouveau `currencyFromFile`), sinon un relevé muet aurait été refusé sur un compte en **DJF** et un relevé EUR accepté sur un compte USD. **Solde de clôture** : repris à l import (`statement_balance`, sa date, l écart), là où `updateStatementBalance` existait depuis la vague #70 sans appelant — c est cette donnée qui rend l état de rapprochement (223) comparable à la banque. Preuves : six fichiers dans `src/lib/__tests__/fixtures/` + README des positions de la norme, **23 tests** de lecteurs (mêmes 3 opérations dans les 6 fichiers, `0102D120,00` = 120) et **8 tests** d import (refus USD/EUR sans rien écrire, muet accepté en DJF, solde repris avec son écart, OFX de bout en bout) ; **1 446 tests**, tsc, oxlint, i18n, knip, build. **Phase 0 : la production est déployée** (29 migrations 189 → 227, 0 erreur ; 369 tables, 343 fonctions, `plpgsql_check` 0 erreur, trigger atteignable OK, garde de société **0 exposée sans contrôle**, **96/96 RPC de l écran présentes**), AUD-B00 sur le cloud par le chemin réel (712 comptes, 10 journaux, 1 exercice, société et utilisateur supprimés), et un **piège de retour arrière mesuré** : une restauration ne rend pas les ACL — l image Supabase re-accorde `anon`/`authenticated` à la création — donc rejouer le `REVOKE` global de la 78 après toute restauration. Reste de la phase 0 : les 14 parcours à l écran (P0-08), après déploiement du front. |
| 23/09 23h45 | **H02 fait — et le défaut n'était pas celui annoncé : c'est le contrôle d'isolation lui-même qui ne prouvait rien sur 7 tables.** Le plan disait « 7 tables non visibles par leur propre société, politique manquante ou table de service ». Mesure sur base neuve (202 migrations, 0 erreur) : `crm_campaign_recipients`, `distribution_grill_lines`, **`invoice_lines`**, `pick_list_lines`, `project_task_tags`, `purchase_request_lines`, `service_ticket_messages`. Aucune politique ne manque : les sept sont des tables de lignes dont la RLS passe par le **parent** (`EXISTS (SELECT 1 FROM invoices p WHERE p.id = invoice_lines.invoice_id AND p.tenant_id = current_tenant_id())`). Le semeur générique de `105_rls_tests.sql` remplit toute colonne uuid obligatoire par `gen_random_uuid()` : la ligne fille pointait vers un parent **inexistant**, donc elle était invisible — y compris à sa propre société. Conséquence, et c'est là le vrai défaut : sur ces sept tables le contrôle « aucune ligne de la société B n'est visible » était vert **parce que rien n'y était visible**, pas parce que la RLS tenait — un garde-fou faussement vert de plus, sur `invoice_lines`, table de lignes de facture. **Correctif du test** : une passe de rattachement recolle chaque clé étrangère simple (une colonne, uuid, parent lui-même semé) vers la ligne de la table parente de la **même** société ; et le contrôle final **échoue** désormais dès qu'une table alimentée reste invisible à sa propre société. **Rouge d'abord** : `FAIL : 7 table(s) alimentée(s) invisible(s) à leur propre société` (340 tables, 333 visibles). **Vert après** : **340 tables, 340 alimentées, 340 visibles par leur propre société, 0 fuite**, en-tête `x-tenant-id` falsifié compris — l'isolation est maintenant prouvée sur les 340, contre 333 auparavant. Vérifié au passage : l'insertion est gardée par le même `EXISTS` (`WITH CHECK` sur `invoices`), donc une société ne peut pas attacher une ligne au document d'une autre — il n'y avait pas de trou exploitable derrière. |
| 24/09 01h30 | 🔴 **H09 fait — un visiteur non connecté pouvait appeler 332 des 389 fonctions, et trois d'entre elles s'exécutaient vraiment.** Mesure sur base neuve (202 migrations) : **332 fonctions de `public` exécutables par `anon`**, dont **231 SECURITY DEFINER** et **75 des 96 RPC de l'écran**. Personne n'avait écrit ces droits : `CREATE FUNCTION` accorde EXECUTE à PUBLIC par défaut, et l'image Supabase ajoute un GRANT `anon` par `ALTER DEFAULT PRIVILEGES`. La migration **78** avait révoqué `anon` en son temps — les 149 migrations suivantes l'ont défait, fonction après fonction : c'est une dérive, pas un oubli, et c'est pourquoi le correctif ne pouvait pas être une simple révocation de plus. **Rouge prouvé en appelant, pas en lisant** : sous le rôle `anon`, sans aucun jeton, `auto_revoke_expired_auditors()` → 0, `generate_recurring_tasks()` → ok, `cleanup_expired_idempotency()` → 0. Trois fonctions SECURITY DEFINER qui écrivent, lancées par n'importe qui. (`disable_2fa()` refusait déjà : « No authenticated user » — la garde était dans le corps, pas dans le droit.) **228** : `REVOKE EXECUTE … FROM PUBLIC` et `FROM anon` sur tout le schéma, avec **deux exceptions inscrites**. La première a été trouvée en cassant le produit avant de le réparer : sans `current_tenant_id()`, la page d'inscription répond « permission denied for function current_tenant_id » — les politiques RLS des référentiels lisibles sans connexion (currencies, legislation_packs, tax_rates) l'appellent. La seconde est `available_signup_countries()`, la liste des pays ouverts à l'inscription. **Ce que la mesure a corrigé dans le plan** : l'anti-dérive prévue (« privilèges par défaut ») **ne marche pas**. `ALTER DEFAULT PRIVILEGES … REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC` est sans effet : la fonction créée ensuite a `proacl = NULL`, c'est-à-dire le droit par défaut de PostgreSQL — EXECUTE à PUBLIC — vérifié dans deux sessions distinctes. Le droit PUBLIC de `CREATE FUNCTION` n'est pas un GRANT stocké, on ne peut pas le retirer d'avance ; la révocation par défaut ne mord que sur le GRANT `anon`. Le garde-fou permanent est donc **`ci/check_anon_grants.sql`** (et non `check_definer_grants.sql`, nom qui décrivait mal la chose), avec registre d'exceptions au même contrat que `check_tenant_guard` : une ligne périmée fait échouer la CI. Il passe **avant** les suites, qui créent leur propre outillage dans `public`. **Preuves** : contrôle **rouge** (387 fonctions hors registre, état d'avant rendu à la main) puis **vert** (389 examinées, 2 exposées, toutes inscrites) ; `228_function_grants_tests.sql` **T01, T02, T03, T06, T07 rouges avant**, **7/7 verts après**, dont deux non-régressions qui empêchent la révocation d'être trop large (anon lit toujours devises, banques et pays ; les RPC de l'écran restent ouvertes à `authenticated`) ; anon **332 → 2**, authenticated **362 inchangé** ; **les 33 autres suites SQL restent vertes** sur la même base. |
| 24/09 02h10 | **Le contrôle H09 a d'abord fait tomber la CI — et il avait raison, sur des fonctions qui ne sont pas les nôtres.** Vert en local, rouge en CI : **23 fonctions `plpgsql_check_*` appelables sans connexion**. La cause est un ordre, pas un droit : la CI installe l'extension `plpgsql_check` **après** avoir appliqué les migrations, donc la 228 ne pouvait pas la révoquer, et l'extension arrive avec `EXECUTE` à PUBLIC. En local l'extension n'est pas installée : la base de CI et la base locale ne disent pas la même chose, exactement comme la copie de production et la CI l'avaient déjà montré pour les ACL. **Correctif du contrôle** : les fonctions appartenant à une extension (`pg_depend`, `deptype = 'e'`) sont écartées du corpus — elles ne sont pas écrites ici et arrivent avec leurs droits —, et le **nombre d'écartées est affiché** pour que l'exclusion se voie au lieu de se taire (77 en local : pgcrypto, uuid-ossp, pg_trgm). Installer une extension dans `public` reste un acte délibéré, qui se relit pour lui-même. Re-vérifié après correction : contrôle **rouge à 310** fonctions quand on rend le droit PUBLIC à la main, **vert** après la 228 (312 examinées, 77 écartées, 2 exposées et inscrites) ; **7/7** scénarios de la 228. |
| 24/09 03h00 | 🔴 **H06 fait — la séparation des tâches ne séparait rien, et personne ne pouvait le voir.** Le plan demandait un scénario « avec `enforce_segregation`, l'auteur d'une écriture ne peut pas la valider ». En l'écrivant, le contrôle s'est révélé **creux** : `check_segregation_of_duties` compare `journal_entries.created_by` au valideur, mais **`created_by` n'est jamais renseigné** — pas de défaut de colonne, pas de trigger, et `post_journal_entry`, seul chemin de saisie de l'application, ne l'écrit pas ; la fonction rend donc `true` sans rien comparer. Mesuré rouge : option activée, l'auteur valide sa propre écriture sans un mot (T03), et `validated_by`/`validated_at` — colonnes existantes — ne sont écrits par **aucune** fonction de la base : ni l'auteur ni le valideur d'une écriture n'étaient enregistrés nulle part. **232** pose l'auteur à l'insertion et le valideur à la validation (piste d'audit utile bien au-delà de la séparation), et limite la règle aux écritures **saisies** (D-13). Le point délicat était de reconnaître une écriture saisie sans toucher aux **21 fonctions** qui en produisent : la réponse est la **pile d'appel** (`GET DIAGNOSTICS … PG_CONTEXT`) — une écriture est saisie si la pile cite `post_journal_entry` ou ne cite **aucune** fonction PL/pgSQL (insertion directe du client). Toute fonction génératrice, y compris celles qu'on écrira demain, donne donc une écriture automatique sans rien déclarer. Sans cette limite, cocher l'option rendrait la **facturation impossible** : le trigger de validation de facture crée et valide l'écriture d'un seul mouvement (T07 le fige). Deuxième trou fermé au passage : **saisir et valider d'un seul geste** (`post_journal_entry` avec `status = 'posted'`) est refusé quand l'option est active — sinon la règle se contourne en cochant « comptabilisée » à la saisie (T05) — et le drapeau `is_manual` est **calculé, jamais accepté du client**, ni à l'insertion ni par une mise à jour (T08). **Preuves** : T01, T02, T03, T05 **vus rouges** avant, **8/8 verts** après ; **37 suites SQL** rejouées vertes sur la même base. **Et le contrôle de la 228 a mordu tout de suite** : la fonction de trigger créée par la 232 arrivait exécutable par PUBLIC, `check_anon_grants` l'a refusée, la 232 la révoque dans sa propre ligne — c'est exactement la discipline que la 228 annonçait. **Ce qui reste ouvert** : l'option `enforce_segregation` n'est exposée par **aucun écran** (elle n'existe que dans les types générés) — elle ne peut être activée qu'en base. |
| 24/09 04h15 | 🔴 **H11 fait — le contrôle d'intégrité du journal fiscal criait au loup à chaque appel.** Le plan disait « `verify_nf525_chain` ne comparait pas le maillon précédent ». Revérifié : c'est **faux depuis la 154**, le chaînage est bien comparé. Le vrai défaut est ailleurs et il est pire. Mesure sur trois événements écrits à la suite, sans la moindre suppression : **« 3 sauts, chaîne invalide »**, avec `after_id = before_id = 4516` et `missing_count = -1`. La cause tient en une ligne : `v_last_id := v_rec.id` était exécutée **avant** le contrôle de continuité, qui comparait donc la ligne à elle-même (`id <> id + 1`, toujours vrai). Le journal NF-525 d'une société saine se déclarait altéré à chaque vérification — et un contrôle qui hurle en permanence ne protège rien, puisque plus personne ne distingue la vraie altération. **Second défaut, de conception** : `chain_valid` exigeait zéro saut d'identifiant. Or un saut survient sans aucune suppression — **mesuré** : `log` 4519, transaction annulée sur 4520, `log` 4521 : la séquence consomme son numéro même en cas de ROLLBACK. Même l'arithmétique corrigée, la chaîne aurait été déclarée invalide à la première transaction annulée de la journée. La **233** rend donc `chain_valid` au seul chaînage (le `previous_hash` d'un événement doit être le `current_hash` du précédent, et le contenu doit redonner son hachage) et garde les sauts comme **signal** détaillé et expliqué. **Trouvé en chemin, et c'est une bonne nouvelle** : le journal est **inaltérable par trigger** — `DELETE` et `UPDATE` sont refusés même sous le rôle `postgres` (« Le journal d'événements est inaltérable »). Les scénarios d'altération ont donc dû neutraliser les triggers (`session_replication_role = replica`) pour se mettre dans la peau de qui accède directement à la base : c'est le seul modèle de menace où un chaînage a du sens. T06 fige cette première barrière. **Limite dite plutôt que cachée** : supprimer le **dernier** maillon ne casse aucun chaînage, faute de successeur ; seul un ancrage extérieur (compteur scellé, horodatage tiers) le verrait, et `gap_count` n'y suffit pas puisqu'un ROLLBACK en produit aussi. **Preuves** : T01, T02, T03, T05 **vus rouges** sur la version d'avant (restaurée pour l'occasion), **6/6 verts** après. **Et deux dettes de la branche réparées au passage** : les types générés ne contenaient pas `is_manual` (232) — régénérés sur une base construite depuis l'arbre **commité** seul, pour ne pas emporter les migrations en cours d'une autre session — et la clé `settings.api.rateLimit`, appelée par `ApiDocsPage` sans exister, faisait échouer le job i18n de tout le monde : la clé est posée dans l'espace `settings` (fr/en/ar) et l'appel corrigé. |
